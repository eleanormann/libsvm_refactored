...
class svm_predict {
  ...
  private static SvmPrintInterface svmPrintString = SvmPrinterFactory
      .getPrinter(PrintMode.STANDARD);
  ...
  protected static void predict(BufferedReader input, DataOutputStream output, SvmModel model,
    ...
    if (predict_probability == 1) {
      if (svmType == SvmType.epsilon_svr || svmType == SvmType.nu_svr) {
        ...
        svm.setModelLabels(model, labels);
    ...
    if (svmType == SvmType.epsilon_svr || svmType == SvmType.nu_svr) {
      ...
      svm_predict
          .info(String.format("Squared correlation coefficient = %s (regression)", rSquared));
    } else {

      double accuracy = (double) correct / total * 100;
      svm_predict.info(String.format("Accuracy = %s%s (%s/%s) (classification)", accuracy, "%",
          correct, total));
    }
  }
  ...
  private void run(String[] argv) throws IOException {
    ...
    try (BufferedReader input = createReader(argv[i]);
