...
//
public class Cache {
	private final int numDataItems;
	private long size;
	...
	public Cache(int l_, long size_) {
		numDataItems = l_;
		size = size_;
		head = new head_t[numDataItems];
		for (int i = 0; i < numDataItems; i++)
			head[i] = new head_t();
		size /= 4;
		size -= numDataItems * (16 / 4); // sizeof(head_t) == 16
		size = Math.max(size, 2 * (long) numDataItems); // cache must be large enough for
												// two columns
		lru_head = new head_t();
